...
- **Universe:** starts at near level of detail so real source-backed molecules are visible in the first viewport. Spatial cluster regions are interactive and synchronized with camera pan/zoom.
...
- Near Universe loads only the bounded visible source structures.
- Cluster renders the selected region in the shared scene.
- Focus renders one high-detail interactive molecule.
- Compare reuses the scene for two to four structures.

The initial generated-catalog metadata window is deterministic and bounded; it is not the searchable catalog ceiling. Search and indexed browse operate across all 1,552 imported records, then hydrate only the selected shard/entity and requested structure through bounded caches.
