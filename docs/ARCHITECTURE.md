...
Synthesis now has five deliberately separate layers: catalog-wide discovery coverage, candidate/source-content extraction, public-alpha pending draft assembly, private canonical route review, and the reviewed/verified public canonical projection. The first layer covers all 1,552 molecular identities. The accepted extraction run terminalizes all 14,897 molecule–evidence associations. Source-content inspection stores hashes, access/rights outcomes, structured locator tokens, and generated non-quoting review cues only; it stores no source body or procedure window. Exact-target ORD segments may enter public alpha only through the independent identity/locator/rights/safety gate.

Six migrated canonical route drafts remain private and pending. `getSynthesisRoutePublicationDecision` combines canonical validation, applicability, review, route/source reuse, and safety gates; the current canonical public index is empty. Public-alpha graphs use a separate schema, index, loader, and UI. The loader validates exact identity, pending state, independent-redraw rights, safe paths, citations, topology, and the absence of operational keys before rendering; failure returns to coverage only. `published-synthesis-route.ts` remains the independent reviewed-route gate. Legacy Dossier, Academy, Instructor, mission, and evidence-card data cannot bypass either projection.
