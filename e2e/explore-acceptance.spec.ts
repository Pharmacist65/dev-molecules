...
test("full-index search creates reload-safe molecule and compare permalinks", async ({ page }) => {
  ...
  const metforminResult = page.locator(`[data-catalog-result="${METFORMIN_ID}"]`);
  await expect(metforminResult).toBeVisible();
...
test("Universe camera input defers viewport membership work and performs one bounded settled rebuild", async ({ page }) => {
  await page.goto("/#universe", { waitUntil: "domcontentloaded" });
  ...
  expect(await readMolecularSceneOperationCounts(universe.canvas)).toEqual(operationsBefore);
...
test("student-first Explore covers spatial Universe, comparison, Passport and Reviewer evidence", async ({ page }, testInfo) => {
  ...
  const nearMembershipBeforeInteraction = await near.scene.getAttribute(
    "data-visible-molecules",
  );
  const nearOperationsBeforeInteraction = await readMolecularSceneOperationCounts(near.canvas);
  ...
  expect(await readMolecularSceneOperationCounts(near.canvas)).toEqual(
    nearOperationsBeforeInteraction,
  );
  await expect
    ...
    .not.toBe(nearMembershipBeforeInteraction);
  await expect
    ...
    .toEqual({
      loadMolecules: nearOperationsBeforeInteraction.loadMolecules,
      updateVisibleMolecules: nearOperationsBeforeInteraction.updateVisibleMolecules + 1,
      rebuildScene: nearOperationsBeforeInteraction.rebuildScene + 1,
    });
