...
import { resolveExploreClusterLabelLayout } from "@/lib/application/explore-cluster-layout";
...
interface SceneTelemetry {
  ...
  hoveredAtom: MolecularSceneAtom | null;
}
...
function cameraZoom(camera: MolecularSceneCamera) {
  ...
  return clamp(DEFAULT_CAMERA_DISTANCE / Math.max(distance, 0.01), 0.45, 4);
}
...
export function MoleculeUniverse({
  ...
  const [indexedResults, setIndexedResults] = useState<readonly IndexedCatalogHit[]>([]);
  const [indexedBrowsePage, setIndexedBrowsePage] =
    useState<IndexedCatalogBrowsePage | null>(null);
  ...
  const indexedSearchActive = normalizedQuery.length >= 2;
  const indexedDisplayRecords = indexedSearchActive
    ? indexedResults
    : (indexedBrowsePage?.records ?? []);
  ...
  const clusters = useMemo<UniverseCluster[]>(() => {
    ...
    >();
    for (const molecule of visibleMolecules) {
  ...
  }, [activeLensId, locale, sceneViewportAspect, unclassifiedLabel, visibleMolecules]);
  const unprojectedMoleculeCount = visibleMolecules.filter(
  ...
  const clusterMolecules = useMemo(
    () =>
      effectiveClusterName
        ...
        : [],
    [activeLensId, effectiveClusterName, unclassifiedLabel, visibleMolecules],
  );
  ...
  const lodLevel: ExploreLodLevel =
    level === "compare"
      ...
      : getExploreLodLevel(
          level,
          searchFocusedId ? Math.max(universeZoom, 1.08) : universeZoom,
        );
  const sceneLevel =
    ...
    () =>
      level === "focus"
        ...
        : level === "compare"
          ...
          ? clusterMolecules
          : visibleMolecules,
    [clusterMolecules, comparisonMolecules, level, selectedMolecule, visibleMolecules],
  );
  const candidateIds = useMemo(() => {
    ...
    const viewportIds = selectViewportSceneCandidateIds({
      candidates: projected.flatMap((molecule) =>
        molecule.position ? [{ id: molecule.id, position: molecule.position }] : [],
      ),
      camera: viewportSelectionCamera,
      viewportAspect: sceneViewportAspect,
      limit: MAX_NEAR_LOD_MOLECULES,
    });
    ...
    return emphasizedId && intendedIdSet.has(emphasizedId) && !viewportIds.includes(emphasizedId)
      ? [emphasizedId, ...viewportIds].slice(0, MAX_NEAR_LOD_MOLECULES)
      : viewportIds;
  }, [
  ...
  const requestedSceneIds = useMemo(
    ...
    [candidateIds, lodLevel, selectedMolecule?.id],
  );
  ...
  async function loadIndexedBrowse(offset: number) {
    if (!indexedCatalog) return;
    const requestId = indexedRequestRef.current + 1;
    indexedRequestRef.current = requestId;
    setIndexedStatus("loading");
    setIndexedErrorMessage("");
    try {
      const page = await indexedCatalog.browse(offset, INDEXED_RESULT_LIMIT);
      if (requestId !== indexedRequestRef.current) return;
      setIndexedBrowsePage(page);
      setIndexedResults([]);
      setIndexedStatus("ready");
    } catch (error) {
      if (requestId !== indexedRequestRef.current) return;
      setIndexedErrorMessage(error instanceof Error ? error.message : "Catalog browse failed.");
  ...
  function showCluster() {
    if (!selectedClusterName) return showUniverse();
  ...
  function resetCamera() {
    ...
    const resetState = level === "universe" ? STUDENT_UNIVERSE_CAMERA : DEFAULT_MOLECULAR_SCENE_CAMERA;
    applyCamera(resetState);
    if (level === "universe") setUniverseZoom(STUDENT_UNIVERSE_ZOOM);
  }
  ...
  useEffect(() => {
    if (!indexedCatalog || !indexedPanelOpen) return undefined;
    const requestId = indexedRequestRef.current + 1;
    indexedRequestRef.current = requestId;
    ...
    const timer = window.setTimeout(() => {
      setIndexedStatus("loading");
      setIndexedErrorMessage("");
      const request = indexedSearchActive
        ? indexedCatalog.search(query, INDEXED_RESULT_LIMIT).then((records) => ({
            records,
            page: null,
          }))
        : indexedCatalog.browse(0, INDEXED_RESULT_LIMIT).then((page) => ({
            records: [] as readonly IndexedCatalogHit[],
            page,
          }));
      void request
        .then(({ records, page }) => {
          if (requestId !== indexedRequestRef.current) return;
          setIndexedResults(records);
          setIndexedBrowsePage(page);
          setIndexedStatus("ready");
        })
        .catch((error: unknown) => {
          if (requestId !== indexedRequestRef.current) return;
          setIndexedResults([]);
          setIndexedBrowsePage(null);
          setIndexedErrorMessage(error instanceof Error ? error.message : "Catalog search failed.");
          setIndexedStatus("error");
        });
    }, indexedSearchActive ? 180 : 0);
    return () => window.clearTimeout(timer);
  }, [indexedCatalog, indexedPanelOpen, indexedSearchActive, normalizedQuery, query]);
    ...
    >
      <header className={styles.header}>
        ...
        <div className={styles.headerSummary} aria-label={t("explore.atlasSummary")}>
          <span>{t("explore.moleculeCount", {
            visible: visibleMolecules.length,
            total: catalogRecordCount ?? molecules.length,
          })}</span>
        ...
        >
          ...
          <div className={styles.searchControl}>
            ...
            <input
              ...
              autoComplete="off"
              aria-controls={indexedCatalog ? `${searchId}-catalog-results` : undefined}
              onFocus={() => {
                if (indexedCatalog) setIndexedPanelOpen(true);
              }}
              onChange={(event) => {
                if (indexedCatalog) setIndexedPanelOpen(true);
          ...
          {indexedCatalog && indexedPanelOpen ? (
            <div
              id={`${searchId}-catalog-results`}
              className={styles.indexedResults}
              data-indexed-status={indexedStatus}
            >
              <header>
                <strong>{catalogCopy.results}</strong>
                <span>
                  {interpolateIndexedCopy(catalogCopy.pageSummary, {
                    shown: indexedSearchActive
                      ? indexedDisplayRecords.length
                      : (indexedBrowsePage?.offset ?? 0) + indexedDisplayRecords.length,
                    total: indexedBrowsePage?.total ?? catalogRecordCount ?? molecules.length,
                  })}
                </span>
              </header>
              {indexedStatus === "loading" || indexedStatus === "hydrating" ? (
                <p role="status">
                  {indexedStatus === "hydrating" ? catalogCopy.hydrating : catalogCopy.loading}
                </p>
              ) : indexedStatus === "error" ? (
                <p role="alert">{catalogCopy.unavailable}</p>
              ) : normalizedQuery.length === 1 ? (
                <p>{catalogCopy.typeMore}</p>
              ) : indexedStatus === "ready" && indexedDisplayRecords.length === 0 ? (
                <p>{catalogCopy.noResults}</p>
              ) : (
                <ul role="listbox" aria-label={catalogCopy.results}>
                  {indexedDisplayRecords.map((record) => (
                    <li key={record.id}>
                      <button
                        type="button"
                        role="option"
                        aria-selected="false"
                        data-catalog-result={record.id}
                        onClick={() => void selectIndexedResult(record)}
                      >
                        <strong>{record.preferredName}</strong>
                        <span>
                          {interpolateIndexedCopy(catalogCopy.resultMeta, {
                        ...
                        {record.aliases.length > 0 ? (
                          <small>
                            {catalogCopy.aliasPrefix}: {record.aliases.slice(0, 2).join(" · ")}
                          </small>
                        ) : null}
                      </button>
                    </li>
              ...
              {!indexedSearchActive && indexedBrowsePage ? (
                <footer>
                  <button
                    type="button"
                    disabled={indexedBrowsePage.previousOffset === null || indexedStatus === "loading"}
                    onClick={() => {
                      if (indexedBrowsePage.previousOffset !== null) {
                        void loadIndexedBrowse(indexedBrowsePage.previousOffset);
                      }
                    }}
                  >
                  ...
                  <button
                    type="button"
                    disabled={indexedBrowsePage.nextOffset === null || indexedStatus === "loading"}
                    onClick={() => {
                      if (indexedBrowsePage.nextOffset !== null) {
                        void loadIndexedBrowse(indexedBrowsePage.nextOffset);
                      }
                    }}
                  >
                    {catalogCopy.next} →
      ...
      >
        ...
        >
          <SharedMolecularScene
            ...
            onStatusChange={(detail) => setTelemetry((current) => ({ ...current, status: detail.status, loadedCount: detail.loadedMoleculeCount }))}
            ...
            onAtomHover={(atom) => setTelemetry((current) => ({ ...current, hoveredAtom: atom }))}
          />
            ...
            <div className={styles.clusterLayer} data-near={lodLevel === "near" ? "true" : "false"}>
              {lodLevel === "far" ? (
                ...
                <div
                  ...
                  style={{
                    transform: `translate3d(${universePan.x}px, ${universePan.y}px, 0) scale(${universeZoom / STUDENT_UNIVERSE_ZOOM})`,
                  }}
                >
              ...
              {lodLevel === "near" && telemetry.hoveredAtom ? (
                <output className={styles.moleculeHoverLabel} aria-live="polite">
                  {telemetry.hoveredAtom.moleculeName} · {telemetry.hoveredAtom.element}{telemetry.hoveredAtom.atomIndex + 1}
                </output>
              ) : null}
