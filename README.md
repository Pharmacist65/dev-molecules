...
- **Explore** — one shared Three.js/WebGL scene supports Universe → cluster → molecule focus and a 2–4 molecule comparison view. Near Universe renders source structures through bounded progressive detail; focus provides ball-and-stick and space-filling representations, 2D/3D switching, rotate, pan, zoom, reset, hydrogen visibility, and atom hover/selection.
...
The original 15 curated molecule records remain regression fixtures and teaching-content anchors, not a schema or product ceiling. Explore keeps a bounded representative resident window, searches the complete 1,552-record static index, hydrates one matching shard/entity on demand, and loads individual structures through bounded caches.
